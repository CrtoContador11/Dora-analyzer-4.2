...
const SavedForms: React.FC<SavedFormsProps> = ({ formData, questions, categories, language, onUpdateForm, onDeleteForm }) => {
  ...
  const handleSaveChanges = () => {
    if (selectedFormIndex !== null && selectedFormIndex >= 0 && selectedFormIndex < formData.length) {
      const updatedForm = {
  ...
  const handleGeneratePDF = () => {
    if (selectedFormIndex !== null && selectedFormIndex >= 0 && selectedFormIndex < formData.length && chartRef.current) {
  ...
  const generateChartData = (selectedData: FormDataType | undefined) => {
    if (!selectedData || !selectedData.answers) {
      return {
        labels: [],
        datasets: [{
          label: '',
      ...
      datasets: [
        {
          ...
          data: categories.map((category) => {
            ...
            const validScores = categoryScores.filter(score => score !== undefined);
  ...
  const selectedForm = selectedFormIndex !== null && selectedFormIndex >= 0 && selectedFormIndex < formData.length 
    ? formData[selectedFormIndex] 
    : undefined;
          ...
          {formData.map((data, index) => (
            ...
            >
              ...
              <p className="text-xs text-gray-600">
                {new Date(data.date).toLocaleDateString(language === 'es' ? 'es-ES' : 'pt-BR')}
              </p>
            </div>
          ))}
        </div>
          ...
          {selectedForm && (
            <div className="bg-gray-50 p-4 rounded-lg">
              <div className="flex flex-col sm:flex-row justify-between items-center mb-4">
                ...
                <div className="flex flex-wrap justify-center sm:justify-end">
                  ...
                  <button
                    ...
                    onClick={() => onDeleteForm(selectedForm.date)}
                  >
              ...
              <div className="mb-8 overflow-x-auto">
                <Bar 
                  ref={chartRef} 
                  data={generateChartData(selectedForm)} 
                          ...
                          {isEditing ? (
                            ...
                            <p className="text-gray-600">
                              {question.options.find(
                                (option) => option.value === selectedForm.answers[question.id]
                              )?.text[language]}
                            </p>
                          )}
                          {isEditing ? (
                            ...
                            <p className="text-gray-600 mt-2">
                              ...
                              {selectedForm.observations[question.id] || '-'}
                            </p>
                          )}
                        </div>
                      ))}
