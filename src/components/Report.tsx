  ...
  return (
    <div className="bg-white shadow-md rounded px-4 sm:px-8 pt-6 pb-8 mb-4 w-full max-w-4xl mx-auto">
      ...
      <div className="mt-8">
        ...
        <p className="text-sm sm:text-base">
          <strong>{language === 'es' ? 'Entidad Financiera:' : 'Entidade Financeira:'}</strong> {selectedForm.financialEntityName}
        </p>
        <p className="text-sm sm:text-base">
          <strong>{language === 'es' ? 'Departamento:' : 'Torre de Serviços:'}</strong> {selectedForm.providerName}
        </p>
        <p className="text-sm sm:text-base">
          <strong>{language === 'es' ? 'Usuario:' : 'Usuário:'}</strong> {selectedForm.userName}
        </p>
        <p className="text-sm sm:text-base">
          <strong>{language === 'es' ? 'Fecha:' : 'Data:'}</strong> {new Date(selectedForm.date).toLocaleDateString(language === 'es' ? 'es-ES' : 'pt-BR')}
        </p>
      </div>
      <button
