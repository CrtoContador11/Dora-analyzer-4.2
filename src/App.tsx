...
const App: React.FC = () => {
  // Language selector is hidden but structure maintained for future use
  const [language, setLanguage] = useState<'es' | 'pt'>('pt');
  ...
  const handleFormSubmit = async (data: FormDataType) => {
    ...
    setSelectedFormIndex(newFormData.length - 1); // Set index to the latest form
  ...
  const handleDeleteForm = (dateToDelete: string) => {
    ...
    setFormData(newFormData);
    if (selectedFormIndex >= newFormData.length) {
      setSelectedFormIndex(Math.max(0, newFormData.length - 1));
    }
  };

  return (
    <div className="min-h-screen bg-gray-100 flex flex-col">
      <Header version="v 4.2" userName={userName} language={language} setLanguage={setLanguage} showLanguageSelector={false} />
